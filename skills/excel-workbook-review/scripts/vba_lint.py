...
def lint_file(path):
    ...
    # Excel generates empty document modules (ThisWorkbook, Sheet1) that carry
    # nothing but Attribute lines. Linting them produces noise about code that
    # does not exist, which buries the findings that matter.
    if not any(
        line.strip() and not line.startswith("Attribute ") and not line.strip().startswith("'")
        for line in raw
    ):
        return []
